...
<mapper namespace="${namespace}">
	...
	<insert id="insert" parameterType="map">
		...
		VALUES ( 
		<#list columns as property>
			#${r"{"}${property.propertyName}, jdbcType=${property.jdbcType}${r"}"}<#if property_has_next>, 
		</#if>
		</#list>
		)
	</insert>
	...
	<update id="update" parameterType="map">
		...
		<set>
			<#list columns as property>
			<if test="${property.propertyName} != null">
				${property.columnName} = #${r"{"}${property.propertyName}, jdbcType=${property.jdbcType}${r"}"}<#if property_has_next>,</#if>
			</if>
			</#list>
		</set>
		<where>
